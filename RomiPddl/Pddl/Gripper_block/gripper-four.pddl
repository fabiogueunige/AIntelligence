(define (problem gripper-four)
    ...
    (:objects 
        ...
        left right                  ; left and right arm gripper
    )
    
    (:init 
        ; rooma_a and room_b are rooms
        (ROOM room_a) (ROOM room_b)
        ; ball 1 to 4 are balls 
        ...
        ; the gripper is initially empty
        (free left) (free right) 
    )
    
    (:goal
        (and 
            (at_ball ball_1 room_b)
            (at_ball ball_2 room_b)
            (at_ball ball_3 room_b)
            (at_ball ball_4 room_b)
            )
    )
